...
"""
    thermal_time(ρc::FT, Δz::FT, κ::FT, scalar_coeff::FT) where {FT}

Returns the thermal timescale for temperature differences across
a typical thickness Δz to equilibrate.

scalar_coeff ρc Δz^2 /κ

"""
function thermal_time(ρc::FT, Δz::FT, κ::FT, scalar_coeff::FT) where {FT}
    return scalar_coeff * ρc * Δz^2 / κ
end
