...
included in the land model.
"""
lsm_aux_types(m::LandModel{FT}) where {FT} = (FT, FT, FT, FT, FT, FT, FT,FT, FT, FT, FT, FT, FT, FT, FT, FT)
...
"""
lsm_aux_domain_names(m::LandModel) =
    (:subsurface, :subsurface, :surface, :surface, :surface, :surface, :surface,    :surface,
    :surface,
    :surface,
    :surface,
    :surface,
...
function lsm_radiant_energy_fluxes!(
    ...
    @. LW_u_soil = ϵ_soil * _σ * p.T_soil_sfc^4 + (1 - ϵ_soil) * LW_d_canopy # double checked
    @. LW_u_snow = ϵ_snow * _σ * p.T_snow_sfc^4 + (1 - ϵ_snow) * LW_d_canopy # check
    # This is a sign inconsistency. Here Rn is positive if towards ground. X_X
    @. R_net_soil += ϵ_soil * LW_d_canopy - ϵ_soil * _σ * p.T_soil_sfc^4 # double checked
    @. R_net_snow += ϵ_snow * LW_d_canopy - ϵ_snow * _σ * p.T_snow_sfc^4 # check
    @. LW_net_canopy =
        ϵ_canopy * LW_d - 2 * ϵ_canopy * _σ * T_canopy^4 + ϵ_canopy * LW_u_soil * (1 - p.snow.snow_cover_fraction) + + ϵ_canopy * LW_u_snow *  p.snow.snow_cover_fraction # check
    @. LW_u = (1 - ϵ_canopy) * LW_u_soil * (1 - p.snow.snow_cover_fraction) + (1 - ϵ_canopy) * LW_u_snow * p.snow.snow_cover_fraction + ϵ_canopy * _σ * T_canopy^4 # check
end
...
function soil_boundary_fluxes!(
    ...
    Soil.Runoff.update_runoff!(p, bc.runoff, p.drivers.P_liq .+ p.snow.water_runoff .* p.snow.snow_cover_fraction .+
        p.excess_water_flux, Y, t, soil)
    @. p.soil.top_bc.water =
        p.soil.infiltration +
        (1 - p.snow.snow_cover_fraction) *
        p.soil.turbulent_fluxes.vapor_flux_liq
    @. p.soil.top_bc.heat =
        (1 - p.snow.snow_cover_fraction) * (-p.soil.R_n + p.soil.turbulent_fluxes.lhf + p.soil.turbulent_fluxes.shf)  +
        p.excess_heat_flux +
...
function ClimaLand.Soil.sublimation_source(::Val{(:canopy, :snow, :soil, :soilco2)}, FT)
