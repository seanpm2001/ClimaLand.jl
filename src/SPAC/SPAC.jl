...
import Base
Base.zero(MonoMLTreeSPAC) = MonoMLTreeSPAC();
Base.size(MonoMLTreeSPAC) = 1;

using Land.EarthSurface: solar_zenith_angle
using Land.ClimaCache: AbstractSPACSystem, BetaFunction, BetaParameterG1, BetaParameterPsoil, GCO₂Mode, MedlynSM, MonoMLTreeSPAC, Soil, VanGenuchten
...
abstract type AbstractSpacModel{FT} <: AbstractModel{FT} end
...
ClimaLSM.name(::AbstractSpacModel) = :spac

"""
    struct SpacModelParameters{
        FT <: AbstractFloat,
        PSE,
    }
...
SpacModelParameters(
    ...
    τc::FT,
    earth_param_set::PSE,
) where {FT, AAM, PSE} = SpacModelParameters{FT, AAM, PSE}(
    κ_soil,
    ρc_soil,
    albedo,
    σS_c,
...
struct SpacModel{
    FT,
    SPAC<: AbstractSPACSystem{FT},
    PS  <: SpacModelParameters{FT},
    ATM <: AbstractAtmosphericDrivers{FT},
    RAD <: AbstractRadiativeDrivers{FT},
    D,
} <: AbstractSpacModel{FT}
    "SPAC model"
    spac::SPAC
...
function SpacModel(;
    spac::ASS,
    parameters::SpacModelParameters{FT, PSE},
    domain::ClimaLSM.Domains.AbstractLSMDomain,
    atmosphere::ATM,
    radiation::RAD,
) where {FT, ASS, PSE, ATM, RAD}
    args = (spac,parameters, atmosphere, radiation, domain)
    SpacModel{FT, typeof.(args)...}(args...)
end



prognostic_types(::SpacModel{FT}) where {FT} = (FT, FT, FT, FT)
prognostic_vars(::SpacModel) = (:W, :T, :Ws, :σS)
auxiliary_types(::SpacModel{FT}) where {FT} = (FT, FT, FT, FT, FT, MonoMLTreeSPAC{FT} )
auxiliary_vars(::SpacModel) =
    (:q_sfc, :evaporation, :turbulent_energy_flux, :R_n, :T_sfc, :spac)

"""
    ClimaLSM.initialize(model::SpacModel{FT}) where {FT}
...
"""
function ClimaLSM.initialize(model::SpacModel{FT}) where {FT}
    ...
    surface_prog_states = map(surface_keys) do (key)
        getproperty(surface_prog.spac, key)
    end

    values = (surface_prog_states..., subsurface_prog.spac.T)
    keys = (surface_keys..., :T)
...
"""
function make_rhs(model::SpacModel{FT}) where {FT}
...
"""
function make_update_aux(model::SpacModel{FT}) where {FT}
...
include("./spac_parameterizations.jl")
