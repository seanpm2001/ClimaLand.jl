...
"""
function data_url(id::Int, state::AbstractString, fields::Vector{String};
    metric::Bool=false,
    hourly::Bool=false,
    ...
    link = link * (metric ? ",metric/" : "/") * (hourly ? "hourly" : "daily") * "/start_of_period/"
...
"""
    get_data(id, state, fields; metric = false, hourly = false)
...
"""
function get_data(id::Int, state::AbstractString, fields::Vector{String};
    metric::Bool=false,
    hourly::Bool=false,
    start::String = "start",
    finish::String = "end")
    url = data_url(id, state, fields, metric=metric, hourly=hourly, start_date = start, end_date = finish)
...
"""
    sitedata_daily(id, state; imp_fields, metric_fields, colnames)
...
- `finish::String`: Optional string to specify ending date of data collection. Default is "end".
"""
function sitedata_daily(id::Int, state::AbstractString;
    imp_fields::Vector{String}=["WTEQ", "SNWD", "PREC"],
    metric_fields::Vector{String}=["RHUMV", "SRADV", "WSPDV", "TAVG"],
    colnames::Vector{String}=["date", "SWE", "z", "precip", "rel_hum_avg", "sol_rad_avg", "wind_speed_avg", "air_temp_avg"],
    start::String = "start",
    finish::String = "end")
    imp_data = get_data(id, state, imp_fields, start = start, finish = finish)
    metric_data = get_data(id, state, metric_fields, metric=true, start = start, finish = finish)
...
"""
    sitedata_hourly(id, state; imp_fields, metric_fields, colnames)
...
- `finish::String`: Optional string to specify ending date of data collection. Default is "end".
"""
function sitedata_hourly(id::Int, state::AbstractString;
    imp_fields::Vector{String}=["WTEQ", "SNWD", "PREC"],
    metric_fields::Vector{String}=["RHUMV", "SRADV", "WSPDV", "TOBS"],
    colnames::Vector{String}=["date", "SWE", "z", "precip", "rel_hum_avg", "sol_rad_avg", "wind_speed_avg", "air_temp_avg"],
    start::String = "start",
    finish::String = "end")
    imp_data = get_data(id, state, imp_fields, hourly=true, start = start, finish = finish)
    metric_data = get_data(id, state, metric_fields, metric=true, hourly=true, start = start, finish = finish)
    data = DataFrames.innerjoin(imp_data, metric_data, on=:Date)
    DataFrames.rename!(data, Symbol.(colnames))
...
function apply_bounds(data::DataFrame, bounds::Dict{Symbol,Tuple{Real,Real}})
    ...
    for colname in names(bounded)
        ...
        bounded[!, Symbol(colname)] = convert(Vector{Union{Missing, coltype}}, bounded[!, Symbol(colname)])
    end
...
function hourly2daily(hourlydata::DataFrame)
    ...
    missingmean(input) = isempty(collect(skipmissing(input))) ? missing : mean(skipmissing(input))
    missingfirst(input) = isempty(collect(skipmissing(input))) ? missing : first(skipmissing(input))
    dailydata = combine(groupby(data, :date),
        [:SWE, :z, :precip] .=> missingfirst,
        [:rel_hum_avg, :sol_rad_avg, :wind_speed_avg, :air_temp_avg] .=> missingmean,
        renamecols=false)
    return dailydata
end
...
function rectify_daily_hourly(daily_data::DataFrame, hourly_data::DataFrame)
    ...
    for var in vars[2:end]
        combined[!, Symbol(var)] .= coalesce.(combined[!, Symbol(var)], combined[!, Symbol(var * "_1")])
    end
...
Day
"""
    apply_limits!(data, var, limits)

Apply a min/max filter on a column of a data frame, and remove outlying data.

# Arguments
- `data::DataFrame`: the data frame over which to apply the filter.
- `var::Symbol`: the column over which to apply the filter.
- `limits::Type{Any, Any}`: The min/max form of the filter to apply to the column.
"""
function apply_filter!(data::DataFrame, var::Symbol, limits::Tuple{Any, Any})
    data = data[(limits[1] .<= data[!, var] .<= limits[2]), :]
end
...
"""
function makediffs(data::DataFrame, Δt::Period;
    ...
    for var in diffvars
        ...
        diffdata[!, Symbol("d"*string(var)*"dt")] = dvar ./ Dates.value.(Second.(dt))
    end
...
"""
#need to check
function rolldata(data, Δt::Period, N::Int;
...
"""
function snowsplit(air_temp::Vector{<:Real}, hum::Vector{<:Real}, precip::Vector{<:Real})
    α = -10.04
    β = 1.41
    γ = 0.09*100 #need to figure out if HUM in decimal 0-1 or 0-100 for this constant!
    #snow_frac = round.(1.0 ./ (1.0 .+ exp.(α .+ β .* air_temp .+ γ .* hum))) #leave-be or round?
    snow_frac = (1.0 ./ (1.0 .+ exp.(α .+ β .* air_temp .+ γ .* hum))) #leave-be or round?
...
"""
    filter_phys!(data; ϵ = 0.2)

Filter unphysical/undesirable data points out of the dataset.
**Note: requires column names to match those of the paper for usage.

# Arguments
- `data::DataFrame`: the cleaned and processed data.
- `ϵ::Real`: a filtering threshold, set to 0.5 cm.
"""
function filter_phys!(data::DataFrame;
    eps::Real = 0.005)
    #these need to be modified according to paper commentary and correct units.
    zero_condition1 = (data[!, :SWE] .< eps) .& (data[!, :z] .< eps) .& (data[!, :dprecipdt] .< eps/86400.0)
...
"""
    prep_data(data; extract_vars, make_snow_split, physical_filter, ϵ)

Prepare a cleaned data stream for model usage.
**Note: Requires column names to match those of the paper for usage.
...
- `ϵ::Real`: a filtering threshold, set to 0.5 cm.
"""
function prep_data(data::DataFrame;
    extract_vars::Vector{Symbol}=[:date, :id, :z, :SWE, :rel_hum_avg, :sol_rad_avg, :wind_speed_avg, :air_temp_avg, :dprecipdt_snow, :dzdt],
    make_snow_split::Bool=true,
    physical_filter::Bool=true,
    eps::Real = 0.005)
    if physical_filter filter_phys!(data, eps = eps) end
    if make_snow_split
        (dprecipdt_snow, dprecipdt_rain) = snowsplit(data[!, :air_temp_avg], data[!, :rel_hum_avg], data[!, :dprecipdt])
