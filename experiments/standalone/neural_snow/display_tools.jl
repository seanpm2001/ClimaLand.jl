...
"""
function siteplot(dates, pred::Vector{<:Real}, truth::Vector{<:Real}, title::String = "test";
    LR::Union{Nothing, Vector{<:Real}}=nothing)
    plot(dates, pred, label = "Neural Model", title = title, xlabel = "Date", ylabel = "z", color = :red)
    if !isnothing(LR)
...
"""
function cor_map(data::DataFrame;
    ...
    M = cor(Matrix(df))
    (n, m) = size(M)
    Plots.heatmap(M, fc=cgrad([:white, :dodgerblue4]), xticks=(1:m, names(df)), xrot=90, yticks=(1:m, names(df)), yflip=true)
...
function show_net_weights(params)
    ...
    display(heatmap(params, aspect_ratio=1, c=cgrad([:red, :white, :blue]), clims=(-lim, lim)))
end