...
h∇ = [
    ClimaCore.Remapping.interpolate(remapper, sv.saveval[k].soil.h∇) for
    k in 1:length(sol.t)
]
R_s = [
    ClimaCore.Remapping.interpolate(remapper, sv.saveval[k].soil.R_s) for
    k in 1:length(sol.t)
]
R_ss = [
    ClimaCore.Remapping.interpolate(remapper, sv.saveval[k].soil.R_ss) for
    k in 1:length(sol.t)
]
θ_sfc = [
    ClimaCore.Remapping.interpolate(
        remapper,
        ClimaLand.Soil.get_top_surface_field(sol.u[k].soil.ϑ_l, surface_space),
    ) for k in 1:length(sol.t)
]
int_ϑ = [similar(sv.saveval[k].soil.h∇) for k in 1:length(sol.t)]
[
    ClimaCore.Operators.column_integral_definite!(int_ϑ[k], sol.u[k].soil.ϑ_l)
    for k in 1:length(sol.t)
];
int_ϑ = [
    ClimaCore.Remapping.interpolate(remapper, int_ϑ[k]) for k in 1:length(sol.t)
]
...
for (i, (field, field_name)) in enumerate(
    ...
    a2 = Axis(fig[2, 1], xlabel = "Longitude", title = "Δ")
    CairoMakie.heatmap!(ax, longpts, latpts, field[end])

    CairoMakie.heatmap!(ax2, longpts, latpts, field[end] - field[1])
