...
root_path = "brazil_longrun_hourly_$(device_suffix)"
...
function setup_prob(t0, tf, Δt; outdir = outdir, nelements = (10, 10, 15))
    ...
    radius = FT(6378.1e3)
    depth = FT(50)
    center_long, center_lat = FT(-60), FT(0)
    delta_m = FT(1_000_000) # in meters
...
top_bc = ClimaLand.Soil.AtmosDrivenFluxBC(atmos, radiation, runoff_model, (:soil,),)
...
short_names = ["swc",  "si", "tsoil", "rain", "soillhf", "soilshf","vflux","sublflux"]
for short_name in short_names
    myvar = get(simdir; short_name)
    times = ClimaAnalysis.times(myvar)
    for t in times[100:120]
        fig = CairoMakie.Figure(size = (800, 600))
        kwargs = ClimaAnalysis.has_altitude(myvar) ? Dict(:z => 1) : Dict()
        tmp = ClimaAnalysis.slice(myvar, time = t; kwargs...)
...
#make timeseries
short_names = ["swc",  "si", "tsoil", "rain", "soillhf", "soilshf","vflux","sublflux"]
times = Array(86400.0:86400.0:1.0368e7)

var_lists = [[] for i in 1:length(short_names)]
for (short_name, var_list) in zip(short_names, var_lists)
    myvar = get(simdir; short_name)
    for t in times
        kwargs = ClimaAnalysis.has_altitude(myvar) ? Dict(:z => 1) : Dict()
        tmp = ClimaAnalysis.slice(myvar, time = t; kwargs...)
        push!(var_list, mean(tmp.data[mask]))
    end
    fig = CairoMakie.Figure(size = (800, 600))
    ax = CairoMakie.Axis(fig[1,1])
    lines!(ax, times, Float64.(var_list), label = short_name)
    CairoMakie.save(joinpath(root_path, "timeseries_$(short_name).png"), fig)
end
